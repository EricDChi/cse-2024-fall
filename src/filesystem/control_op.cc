...
auto FileOperation::remove_file_atomic(inode_id_t id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  auto error_code = ErrorType::DONE;
  const auto block_size = this->block_manager_->block_size();
  ...
  if (inode_p->blocks[inode_p->get_direct_block_num()] != KInvalidBlockID) {
    // we still need to release the indirect block
    std::vector<u8> indirect_block;
    auto read_res = this->block_manager_->read_block_atomic(
        inode_p->blocks[inode_p->get_direct_block_num()],
  ...
  // First we free the inode
  {
    auto res = this->inode_manager_->free_inode_atomic(id, ops);
    if (res.is_err()) {
      error_code = res.unwrap_error();
      goto err_ret;
    }
  ...
  // now free the blocks
  for (auto bid : free_set) {
    auto res = this->block_allocator_->deallocate_atomic(bid, ops);
    if (res.is_err()) {
      return res;
    }
  }
